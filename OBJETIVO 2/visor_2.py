...
from PyQt6.QtGui import QColor, QBrush, QPen, QFont, QPainter, QPainterPath, QPolygonF
...
class PieChartWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        hole_size = size * 0.5
        hole_rect = QRectF(pie_rect.center().x() - hole_size/2, pie_rect.center().y() - hole_size/2, hole_size, hole_size)
...
class VentanaReporte(QDialog):
    def __init__(self, parent, tasks, fecha_corte_qdate, festivos):
        ...
        sum_pct_fisico = 0; sum_pct_esperado = 0; count_metrics = 0
        data_rows = []
        ...
        for t in self.tasks:
            ...
            data_rows.append({
                ...
                "start": t['bl_start'], "end": t['bl_finish'],
                "status": estado
            })
        ...
        left_panel = QVBoxLayout(); left_panel.setSpacing(15)
        row1 = QHBoxLayout()
        row1.addWidget(KPICard("AVANCE FÍSICO REAL", f"{avg_fisico:.1f}%", "#2980B9"))
        row1.addWidget(KPICard("AVANCE ESPERADO", f"{avg_esperado:.1f}%", "#7F8C8D"))
        left_panel.addLayout(row1)

        row2 = QHBoxLayout()
        card_ok = QFrame(); card_ok.setFixedSize(200, 110)
        card_ok.setStyleSheet("background-color: white; border-radius: 12px; border-left: 5px solid #2ECC71;")
        l_ok = QVBoxLayout(card_ok)
        l_ok.addWidget(QLabel("ACTIVIDADES AL DÍA")); l_ok.addWidget(QLabel(str(en_tiempo)))
        l_ok.itemAt(0).widget().setStyleSheet("color: #2ECC71; font-weight: bold; font-size: 11px;")
        l_ok.itemAt(1).widget().setStyleSheet("color: #2C3E50; font-size: 28px; font-weight: 800;")
        sh_ok = QGraphicsDropShadowEffect(); sh_ok.setBlurRadius(15); sh_ok.setYOffset(4); sh_ok.setColor(QColor(0,0,0,30))
        card_ok.setGraphicsEffect(sh_ok)
        
        card_bad = QFrame(); card_bad.setFixedSize(200, 110)
        card_bad.setStyleSheet("background-color: white; border-radius: 12px; border-left: 5px solid #E74C3C;")
        l_bad = QVBoxLayout(card_bad)
        l_bad.addWidget(QLabel("RETRASADAS")); l_bad.addWidget(QLabel(str(atrasadas)))
        l_bad.itemAt(0).widget().setStyleSheet("color: #E74C3C; font-weight: bold; font-size: 11px;")
        l_bad.itemAt(1).widget().setStyleSheet("color: #2C3E50; font-size: 28px; font-weight: 800;")
        sh_bad = QGraphicsDropShadowEffect(); sh_bad.setBlurRadius(15); sh_bad.setYOffset(4); sh_bad.setColor(QColor(0,0,0,30))
        card_bad.setGraphicsEffect(sh_bad)

        row2.addWidget(card_ok); row2.addWidget(card_bad)
        left_panel.addLayout(row2)
        top_layout.addLayout(left_panel)

        chart_container = QFrame()
        chart_container.setStyleSheet("background-color: white; border-radius: 12px; border: 1px solid #E5E7EB;")
        chart_container.setFixedSize(320, 235)
        sh_chart = QGraphicsDropShadowEffect(); sh_chart.setBlurRadius(15); sh_chart.setYOffset(4); sh_chart.setColor(QColor(0,0,0,30))
        chart_container.setGraphicsEffect(sh_chart)
        l_chart = QVBoxLayout(chart_container)
        lbl_g = QLabel("Distribución de Estado")
        lbl_g.setStyleSheet("border: none; font-weight: bold; color: #555;")
        lbl_g.setAlignment(Qt.AlignmentFlag.AlignCenter)
        l_chart.addWidget(lbl_g)
        pie = PieChartWidget(atrasadas, en_tiempo)
        l_chart.addWidget(pie, 1, Qt.AlignmentFlag.AlignCenter)
        ...
        lbl_det.setFont(QFont("Segoe UI", 14, QFont.Weight.Bold))
        lbl_det.setStyleSheet("color: #2C3E50; margin-top: 10px;")
        main_layout.addWidget(lbl_det)

        self.tabla = QTableWidget()
        self.tabla.setColumnCount(8)
        headers = ["Actividad", "Cant. Plan", "Cant. Ejec", "% Real", "% Esperado", "Diferencia", "Estado", "Fin Base"]
        ...
        self.tabla.setStyleSheet("""
            QTableWidget {
                ...
                gridline-color: #F0F0F0; font-family: 'Segoe UI'; font-size: 13px;
                selection-background-color: #D6EAF8; selection-color: black;
            }
            QTableWidget::item { padding: 5px; border-bottom: 1px solid #F0F0F0; color: #000000; }
            QHeaderView::section {
                background-color: #2C3E50; color: white; padding: 8px; border: none; font-weight: bold; font-size: 12px;
            }
        """)
        self.tabla.setColumnWidth(0, 350)
        ...
        for d in data_rows:
            ...
            item_name = QTableWidgetItem(d['name']); item_name.setForeground(QColor("black"))
            self.tabla.setItem(row, 0, item_name)
            self.tabla.setItem(row, 1, QTableWidgetItem(str(d['c_plan'])))
            self.tabla.setItem(row, 2, QTableWidgetItem(str(d['c_real'])))
            item_real = QTableWidgetItem(f"{d['pct_fis']*100:.1f}%")
            if d['pct_fis'] >= 1.0: 
                item_real.setForeground(QColor("#27AE60"))
                item_real.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
            self.tabla.setItem(row, 3, item_real)
            self.tabla.setItem(row, 4, QTableWidgetItem(f"{d['pct_plan']*100:.1f}%"))
            diff = (d['pct_fis'] - d['pct_plan']) * 100
            item_diff = QTableWidgetItem(f"{diff:+.1f}%")
            if diff < -5: item_diff.setForeground(QColor("#C0392B"))
            elif diff > 0: item_diff.setForeground(QColor("#27AE60"))
            self.tabla.setItem(row, 5, item_diff)
            item_st = QTableWidgetItem(d['status'])
            if d['status'] == "RETRASADA":
                item_st.setBackground(QColor("#FDEDEC")); item_st.setForeground(QColor("#C0392B"))
            else:
                item_st.setBackground(QColor("#EAFAF1")); item_st.setForeground(QColor("#27AE60"))
            item_st.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            item_st.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
            self.tabla.setItem(row, 6, item_st)
            self.tabla.setItem(row, 7, QTableWidgetItem(d['end'].strftime("%d/%m/%y")))

        main_layout.addWidget(self.tabla)
        btns_layout = QHBoxLayout(); btns_layout.addStretch()
        btn_close = QPushButton("Cerrar")
        btn_close.setFixedSize(100, 35)
        btn_close.setStyleSheet("QPushButton { background-color: #95A5A6; color: white; border-radius: 5px; font-weight: bold; }")
...
class VisorProjectPro(QMainWindow):
    ...
    def init_ui(self):
        ...
        style_btn = "QPushButton { color: white; border: none; padding: 6px 15px; font-weight: bold; border-radius: 3px; font-size: 13px; } QPushButton:hover { opacity: 0.9; } QPushButton:disabled { background-color: #CCCCCC; color: #666666; }"

        btn_load = QPushButton("📂 Importar Cronograma")
        btn_load.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_load.setStyleSheet(style_btn + "QPushButton { background-color: #107C41; }") 
        ...
        self.btn_cant.setStyleSheet(style_btn + "QPushButton { background-color: #00BFA5; }")
        ...
        self.btn_base = QPushButton("📐 Crear Línea Base")
        self.btn_base.setCursor(Qt.CursorShape.PointingHandCursor)
        self.btn_base.setStyleSheet(style_btn + "QPushButton { background-color: #0078D7; }") 
        ...
        self.btn_corte.setStyleSheet(style_btn + "QPushButton { background-color: #FF8F00; }") 
        ...
        self.btn_reporte.setStyleSheet(style_btn + "QPushButton { background-color: #6200EA; }") 
        ...
        self.btn_reset.setStyleSheet(style_btn + "QPushButton { background-color: #546E7A; }") 
        ...
        self.tree.setStyleSheet(f"QTreeWidget {{ border: none; font-size: 13px; gridline-color: #F0F0F0; background-color: #FFFFFF; color: #000000; }} QTreeWidget::item {{ border-bottom: 1px solid #F0F0F0; padding-left: 5px; color: #000000; }} QTreeWidget::item:selected {{ background-color: #E6F3FF; color: #000000; }} QHeaderView::section {{ background-color: {self.color_header}; color: #000000; border: 1px solid #DDD; height: {self.alto_encabezado}px; font-weight: bold; }}")
    ...
    def activar_linea_base(self):
        if not self.tasks: return
        reply = QMessageBox.question(self, "Crear Línea Base", "Se guardará el estado actual (Fechas y Cantidades Planificadas) como Línea Base.\n¿Continuar?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    ...
    def llenar_tabla(self):
        ...
        for t in self.tasks:
            ...
            if self.modo_corte:
                ...
                # --- AQUÍ ESTÁ EL CAMBIO PARA BORRAR LOS CEROS EN CAPÍTULOS/RESUMENES ---
        ...
        for t in self.tasks:
            for pid in t['predecessors']:
                if pid in coords and t['uid'] in coords:
                    ...
                    poly = QPolygonF([QPointF(c2['x_in'], c2['y']), QPointF(c2['x_in']-5, c2['y']-3), QPointF(c2['x_in']-5, c2['y']+3)])
                    self.scene.addPolygon(poly, QPen(Qt.PenStyle.NoPen), QBrush(QColor(self.color_flecha)))

if __name__ == "__main__":
    app = QApplication(sys.argv)
    visor = VisorProjectPro()
    visor.show()
    sys.exit(app.exec())